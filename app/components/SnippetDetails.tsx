...
const SnippetDetails = ({snippet}:{snippet:Snippit}) => {
    ...
    return (
		<div className="mt-10">
			...
			<Prism showLineNumbers language={snippet.langauge}>{snippet.content}</Prism>
            <div className="flex justify-center mt-10 gap-4">
                <Link href={`/snippets/edit/${snippet.id}`} className="bg-emerald-500 w-36 py-3 rounded-xl text-white text-center">Edit</Link>
                <button onClick={() => setOpen(true)} className="bg-red-500 w-36 py-3 rounded-xl text-white text-center">
                    Delete
                </button>
            </div>

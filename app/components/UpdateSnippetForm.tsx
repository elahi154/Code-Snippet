"use client";
import {  editSnippet } from "@/lib/actions";
import { SNIPPETDATA } from "@/lib/constant";
import { Snippit, Technology } from "@prisma/client";
...
const EditSnippetForm = ({snippet}:{snippet:Snippit}) => {
	const {
		register,
		handleSubmit,
		formState: { errors },
	...
	const handleFormSubmit = async (data: FormFields) => {
		try {
			const langauge = SNIPPETDATA[data.technology].language;
			const res = await editSnippet({
				...data,
                langauge,
